...
const Card = ({ children, className, title, subtitle, action }: { children: React.ReactNode; className?: string; title?: string; subtitle?: string; action?: React.ReactNode }) => (
  <div className={cn('bg-card backdrop-blur-sm rounded-2xl p-6 border border-card-border shadow-lg transition-all duration-300', className)}>
    {(title || action) && (
...
const Sidebar = ({ activeTab, setActiveTab }: { activeTab: string; setActiveTab: (t: string) => void }) => {
  ...
  const menuItems = [
    ...
    { id: 'admin', label: 'Admin Mode', icon: ShieldAlert, path: '/admin' },
  ];
      ...
      <div className="flex items-center gap-4">
            ...
            {showNotifications && (
              <>
                ...
                <motion.div
                  ...
                  className="absolute right-0 mt-3 w-80 bg-card border border-card-border rounded-2xl shadow-lg z-50 overflow-hidden"
                >
                  ...
                  <div className="max-h-96 overflow-y-auto">
                    {notifications.length > 0 ? (
                      notifications.map((n) => (
                        <div key={n.id} className={cn('p-4 border-b border-card-border hover:bg-background/50 transition-all cursor-pointer', !n.read && 'bg-saffron/5')}>
                          <div className="flex gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-text-secondary/50 mt-2">{n.time}</p>
                            </div>
                          </div>
                        </div>
                      ))
                    ) : (
                      <div className="p-10 text-center text-text-secondary">No notifications</div>
                    )}
                  </div>
                  <button 
                    onClick={() => setNotifications([])}
                    className="w-full p-3 text-xs text-text-secondary hover:text-text-primary hover:bg-background/50 transition-all"
                  >
                    Clear all
                  </button>
                </motion.div>
        ...
        <div className="flex items-center gap-3 pl-4 border-l border-card-border">
          <div className="text-right hidden sm:block">
            <p className="text-sm font-medium text-text-primary">{userData.name || 'User'}</p>
            <p className="text-xs text-text-secondary">ID: SF-{userData.phone.slice(-4)}</p>
          </div>
  ...
  return (
    <div className="space-y-8 relative pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <GlowCard className="lg:col-span-1 border-none bg-gradient-to-br from-card to-green-950/20 shadow-lg shadow-green-500/10 group hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-5">
            <ShieldCheck className="w-32 h-32 text-green-500" />
          </div>
          <p className="text-green-500 text-xs font-bold uppercase tracking-widest relative z-10 flex items-center gap-1.5"><ShieldCheck className="w-4 h-4" /> Earnings Protected</p>
          <div className="flex items-center justify-between mt-2 relative z-10">
            <h2 className="text-4xl font-bold text-text-primary tracking-tight">₹14,500</h2>
          </div>
          <p className="text-xs text-text-secondary mt-1 relative z-10">Total wages shielded via policy</p>
          <div className="mt-6 flex items-center gap-2 relative z-10 p-3 rounded-xl bg-green-500/10 border border-green-500/20">
            <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse shrink-0" />
            <span className="text-[10px] font-bold text-green-500 uppercase tracking-wide">Live Auto-Claim Active</span>
          </div>
        </GlowCard>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between p-4 rounded-2xl bg-background/50 border border-card-border">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-3xl font-bold text-text-primary tracking-tight">32°</p>
            ...
            <div className="grid grid-cols-3 gap-2">
              {['Mon', 'Tue', 'Wed'].map((day, i) => (
                <div key={day} className="bg-background/50 rounded-2xl p-4 text-center border border-card-border group hover:border-saffron/30 transition-all duration-300">
                  ...
                  <p className="text-xs font-bold text-text-primary tracking-tight">34°/26°</p>
                </div>
              ))}
            </div>
        ...
        {/* Recent Activity */}
        <Card className="lg:col-span-2" title="Recent Activity" action={<Button variant="ghost" className="text-xs font-bold uppercase tracking-wider text-saffron">History</Button>}>
          <div className="space-y-4">
            {[
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        {/* Breakdown & Risk Scorecard */}
        <div className="lg:col-span-2 space-y-6">
          <GlowCard glowColor="red" className="border-none shadow-lg shadow-red-500/10 p-8 relative overflow-hidden bg-gradient-to-br from-card to-red-950/20">
             <div className="absolute top-0 right-0 p-6 opacity-10">
               <AlertCircle className="w-40 h-40 text-red-500" />
             </div>
             <h3 className="font-bold text-lg text-text-primary mb-6 relative z-10 flex items-center gap-2">
               <span className="w-2 h-2 rounded-full bg-saffron animate-pulse" />
               AI Risk Scorecard
             </h3>
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 relative z-10">
                 <div>
                    <p className="text-text-secondary text-sm font-bold uppercase tracking-wider">Disruption Probability</p>
                    <AnimatedCounter value={28} suffix="%" className="text-5xl font-bold text-saffron mt-1 tracking-tighter" />
                    <div className="mt-4 w-full h-2 bg-card-border rounded-full overflow-hidden">
                       <motion.div initial={{width:0}} animate={{width:'28%'}} transition={{duration:1, delay:0.5}} className="h-full bg-saffron rounded-full"/>
                    </div>
                    <p className="text-text-secondary text-xs font-semibold mt-2">Hazardous: AQI & Heat Alerts</p>
                 </div>
                 <div className="sm:pl-8 sm:border-l border-card-border">
                    <p className="text-text-secondary text-sm font-bold uppercase tracking-wider">Expected Income Loss</p>
                    <div className="flex items-end gap-1 mt-1">
                      <span className="text-2xl font-bold text-red-500/50 mb-1">₹</span>
                      <AnimatedCounter value={userData.dailyEarnings * 3 * 0.28} decimals={0} className="text-5xl font-bold text-red-500 tracking-tighter" />
                    </div>
                    <p className="text-text-secondary text-xs mt-4 leading-relaxed font-medium">
                      Based on 3 projected high-risk days next week relative to your 
                      <span className="text-text-primary"> ₹{userData.dailyEarnings}/day</span> earnings bracket.
                    </p>
                 </div>
             </div>
          </GlowCard>
...
const PayoutSimulationModal = ({ open, onClose }: { open: boolean; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl">
        <div className="bg-blue-600 p-6 text-center">
          <h2 className="text-white font-bold text-xl tracking-tight">Razorpay Instant</h2>
          <p className="text-blue-100 text-sm mt-1">₹400 to SAFAR Worker Account</p>
        </div>
        <div className="p-8 pb-10">
          {step === 0 && (
            <div className="flex flex-col items-center">
              <Loader2 className="w-12 h-12 text-blue-600 animate-spin mb-4" />
              <p className="text-gray-800 font-medium">Validating Smart Contract...</p>
              <p className="text-gray-500 text-xs mt-2 text-center">Connecting to banking partner gateway</p>
            </div>
          )}
          {step === 1 && (
            <div className="flex flex-col items-center">
               <div className="w-12 h-12 rounded-full border-4 border-blue-600 border-t-transparent animate-spin mb-4" />
               <p className="text-gray-800 font-medium">Processing UPI Transfer...</p>
               <p className="text-gray-500 text-xs mt-2">VPA: *********@okicici</p>
            </div>
          )}
          {step === 2 && (
            <div className="flex flex-col items-center">
               <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring" }} className="w-16 h-16 bg-green-500 rounded-full flex items-center justify-center mb-4 shadow-lg shadow-green-500/30">
                 <CheckCircle2 className="w-8 h-8 text-white" />
               </motion.div>
               <p className="text-gray-800 font-bold text-xl">Payment Successful!</p>
               <p className="text-gray-500 text-sm mt-2 text-center">₹400 has been instantly credited to your linked bank account.</p>
               <Button onClick={onClose} className="mt-8 w-full bg-blue-600 hover:bg-blue-700 text-white rounded-xl py-4 font-bold border-none">Return to Dashboard</Button>
            </div>
          )}
        </div>
      </motion.div>
...
const ClaimsPage = () => {
  ...
  return (
    <div className="space-y-8">
      <PayoutSimulationModal open={showPayout} onClose={() => setShowPayout(false)} />
      <div className="flex items-center justify-between">
        <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <GlowCard className="border-none bg-card backdrop-blur-sm">
          <p className="text-text-secondary text-[10px] font-bold uppercase tracking-widest">Active Claims</p>
          <h2 className="text-3xl font-bold text-text-primary mt-1">1</h2>
        ...
        <GlowCard className="border-none bg-card backdrop-blur-sm">
          ...
          <h2 className="text-3xl font-bold text-text-primary mt-1">₹1,200</h2>
        ...
        <GlowCard className="border-none bg-card backdrop-blur-sm">
          ...
          <h2 className="text-3xl font-bold text-text-primary mt-1">₹400</h2>
          <p className="text-[10px] text-text-secondary/50 font-bold mt-4 uppercase">Per event</p>
        </GlowCard>
      </div>

      {/* NEW: Zero-Paperwork Auto-Claim Workflow Simulation */}
      <Card title="Live Auto-Claim Tracker" subtitle="Processing Event: CLM-9844" className="border-saffron/30 shadow-saffron/10">
        <div className="relative pt-8 pb-4">
          {/* Connecting Line */}
          <div className="absolute top-14 bottom-14 left-[2.25rem] w-0.5 bg-card-border" />
          <motion.div 
            initial={{ height: 0 }} 
            animate={{ height: '70%' }} 
            transition={{ duration: 2, delay: 0.5 }}
            className="absolute top-14 left-[2.25rem] w-0.5 bg-saffron"
          />
          ...
          <div className="space-y-8 relative z-10">
             ...
             <div className="flex items-start gap-6">
                ...
                <div>
                   <h4 className="text-sm font-bold text-text-primary flex items-center gap-2">Parametric Trigger Met <span className="text-[10px] font-normal px-2 py-0.5 rounded-md bg-background/50 text-text-secondary">10:15 AM</span></h4>
                   <p className="text-xs text-text-secondary mt-1">IoT Weather node "Delhi-NCR-01" reported AQI &gt; 350 for 4 straight hours.</p>
                </div>
             </div>
             ...
             <div className="flex items-start gap-6">
                ...
                <div>
                   <h4 className="text-sm font-bold text-text-primary flex items-center gap-2">Location Verified <span className="text-[10px] font-normal px-2 py-0.5 rounded-md bg-background/50 text-text-secondary">10:16 AM</span></h4>
                   <p className="text-xs text-text-secondary mt-1">Confirmed active duty presence in the affected zone via GPS ping.</p>
                </div>
             </div>

             {/* Step 3 */}
             <div className="flex items-start gap-6">
                <div className="w-10 h-10 rounded-full bg-saffron/20 border-2 border-saffron flex items-center justify-center shrink-0 relative">
                   <div className="absolute -inset-2 rounded-full border border-saffron animate-ping opacity-20" />
                   <AlertCircle className="w-5 h-5 text-saffron" />
                </div>
                <div>
                   <h4 className="text-sm font-bold text-saffron flex items-center gap-2">Executing Smart Contract <span className="text-[10px] font-normal px-2 py-0.5 rounded-md bg-saffron/10 text-saffron">In Progress</span></h4>
                   <p className="text-xs text-text-secondary mt-1">Validating policy constraints and calculating ₹400 payout payload.</p>
                   {/* Cool terminal typing effect */}
                   <div className="mt-3 bg-[#0a0a0a] border border-white/10 rounded-lg p-3 font-mono text-[10px] text-green-400 overflow-hidden relative break-all">
                      <motion.div initial={{width: '100%'}} animate={{width: 0}} transition={{duration: 5, ease: "linear"}} className="absolute right-0 top-0 bottom-0 bg-[#0a0a0a]" />
                      &gt; _verify_policy(user_id="SF-9912", event="AQI_CRITICAL")<br/>
                      &gt; Status: ALIGNED<br/>
                      &gt; _init_payout(amount=400, currency="INR")<br/>
                      &gt; [PROCESSING...]
                   </div>
                </div>
             </div>
             ...
             {/* Step 4 */}
             <div className="flex items-start gap-6">
                <div className="w-10 h-10 rounded-full bg-saffron/10 border-2 border-saffron flex items-center justify-center shrink-0">
                   <Wallet className="w-5 h-5 text-saffron" />
                </div>
                <div className="flex-1">
                   <div className="flex items-center justify-between">
                     <div>
                       <h4 className="text-sm font-bold text-text-primary flex items-center gap-2">Ready for Payout</h4>
                       <p className="text-xs text-text-secondary mt-1">Smart contract finalized. Awaiting gateway transfer.</p>
                     </div>
                     <Button 
                        onClick={() => setShowPayout(true)} 
                        className="bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-600/20 py-2 border-none"
                     >
                       Simulate Razorpay Transfer
                     </Button>
                   </div>
                </div>
             </div>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                { label: 'Visibility', value: '2.5 km', icon: Eye },
              ].map((stat, i) => (
                <div key={i} className="bg-background/50 rounded-2xl p-4 border border-card-border">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day, i) => (
                  <div key={day} className="flex items-center justify-between p-4 rounded-xl bg-background/50 border border-transparent hover:border-card-border transition-all">
                    <span className="w-12 text-sm font-medium text-text-secondary">{day}</span>
                    <CloudSun className="w-5 h-5 text-saffron" />
                    <div className="flex-1 mx-8 h-1.5 bg-card-border rounded-full overflow-hidden relative">
                      <div className="absolute top-0 left-1/4 right-1/4 h-full saffron-gradient" />
                    </div>
                    <span className="text-sm font-bold text-text-primary">34° / 26°</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <Card title="Contact Support">
          {submitted ? (
            ...
            <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="space-y-4">
              <div className="space-y-2">
                <label className="text-xs text-text-secondary ml-1 font-medium">Issue Category</label>
              ...
              <div className="space-y-2">
                <label className="text-xs text-text-secondary ml-1 font-medium">Message</label>
                <textarea 
                  rows={4}
                  placeholder="Describe your issue in detail..."
                  className="w-full bg-background/50 border border-card-border rounded-xl px-4 py-3 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-saffron/50 placeholder:text-text-secondary/40"
                />
              </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-4 p-2">
            <div className="flex gap-3">
              ...
              <div className="bg-background/50 rounded-2xl rounded-tl-none p-4 max-w-[80%] border border-card-border">
                ...
                <p className="text-[10px] text-text-secondary/50 mt-2">10:30 AM</p>
              </div>
            </div>
            <div className="flex gap-3 flex-row-reverse">
              ...
              <div className="bg-saffron/10 rounded-2xl rounded-tr-none p-4 max-w-[80%] border border-saffron/20">
                ...
                <p className="text-[10px] text-text-secondary/50 mt-2">10:32 AM</p>
              </div>
            </div>
          </div>
          <div className="pt-4 border-t border-card-border">
            <div className="relative">
              <input 
                ...
                className="w-full bg-background/50 border border-card-border rounded-xl pl-4 pr-12 py-3 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-saffron/50 placeholder:text-text-secondary/40"
              />
    ...
    <div className="space-y-8 relative pb-20">
      <div className="flex justify-between items-center mb-6">
         <div>
            ...
            <p className="text-sm text-text-secondary">Monitor loss ratios, predictive claims, and fraud modules.</p>
         </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <GlowCard glowColor="saffron" className="lg:col-span-1 border-none shadow-lg shadow-saffron/10 p-6 bg-gradient-to-br from-card to-saffron/5">
           <p className="text-text-secondary text-xs font-bold uppercase tracking-widest">Platform Loss Ratio</p>
           <div className="flex items-end gap-2 mt-2">
             <h2 className="text-4xl font-bold text-text-primary tracking-tight">38.4%</h2>
             <span className="text-green-500 text-xs font-bold mb-1">HEALTHY</span>
           </div>
           <p className="text-xs text-text-secondary mt-2">Premiums Collected: ₹1.2M</p>
           <p className="text-xs text-text-secondary">Expected Payouts: ₹460K</p>
           <div className="mt-4 w-full h-2 bg-green-500/20 rounded-full overflow-hidden">
             <div className="h-full bg-green-500 w-[38.4%]" />
           </div>
        </GlowCard>

        <GlowCard glowColor="red" className="lg:col-span-2 border-none shadow-lg shadow-red-500/10 p-6 bg-gradient-to-br from-card to-red-950/20">
           <div className="flex items-center gap-2 text-red-500 mb-2">
             <AlertCircle className="w-5 h-5" />
             <span className="font-bold cursor-default">Predictive Claims Intelligence</span>
           </div>
           <h3 className="text-2xl font-bold text-text-primary mt-1">₹45,000 Projected Exposure</h3>
           <p className="text-text-secondary text-sm mt-2">75% probability of extreme heat event next Tuesday (Zone: Delhi NCR). Expecting ~112 claims.</p>
           <div className="mt-4 flex gap-4">
              <div className="px-4 py-2 border border-card-border bg-background/50 rounded-xl">
                 <p className="text-xs text-text-secondary uppercase font-bold tracking-widest">Risk Category</p>
                 <p className="font-bold text-red-500 mt-1">SEVERE HEAT (&gt;42°C)</p>
              </div>
              <div className="px-4 py-2 border border-card-border bg-background/50 rounded-xl">
                 <p className="text-xs text-text-secondary uppercase font-bold tracking-widest">Recommended Action</p>
                 <p className="font-bold text-saffron mt-1">Raise Premium by 12%</p>
              </div>
           </div>
        </GlowCard>
      </div>

      <Card title="Advanced Fraud Detection" subtitle="Live monitoring of delivery-specific anomalies" className="border-red-500/20 relative overflow-hidden">
            ...
            <div className="p-4 rounded-xl border border-red-500/30 bg-red-500/5">
               <div className="flex justify-between items-start">
                  <div className="flex gap-3">
                     ...
                     <div>
                        <p className="font-bold text-red-500">Flagged: GPS Spoofing Detected</p>
                        <p className="text-sm text-text-secondary mt-1">User SF-1182 submitted a "Heavy Rain" claim. Historical API logs show the device pinged from mock location software.</p>
                        <p className="text-xs font-mono text-red-500/50 mt-2">&gt; verify_location(lat=28.7, long=77.1, mock=True) -&gt; DENIED</p>
                     </div>
                  </div>
                  <Button className="bg-red-500 hover:bg-red-600 text-white shadow-red-500/20 border-none shrink-0 py-2">Reject & Ban</Button>
               </div>
            </div>

            <div className="p-4 rounded-xl border border-saffron/30 bg-saffron/5">
               <div className="flex justify-between items-start">
                  <div className="flex gap-3">
                     <AlertCircle className="w-5 h-5 text-saffron mt-0.5" />
                     <div>
                        <p className="font-bold text-saffron">Flagged: Fake Weather API Discrepancy</p>
                        <p className="text-sm text-text-secondary mt-1">User SF-9943 claimed extreme AQI in Zone B. Ground truth historical sensor map confirms AQI never passed 200 today.</p>
                        <p className="text-xs font-mono text-saffron/50 mt-2">&gt; assert ground_truth.aqi(zone="B") &lt; 350 -&gt; FAILED</p>
                     </div>
                  </div>
                  <Button variant="outline" className="text-saffron shadow-none border-saffron/30 hover:bg-saffron/10 shrink-0 py-2">Manual Review</Button>
               </div>
            </div>
